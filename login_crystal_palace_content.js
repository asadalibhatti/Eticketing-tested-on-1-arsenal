// login_crystal_palace_content.js
console.log('[LOGIN] Crystal Palace login content script loaded on', location.href);

// Check if we're on the correct Crystal Palace login page
(function() {
    if (!window.location.href.includes('cpfc.co.uk/tm-authorize')) {
        console.warn('[LOGIN] Not on Crystal Palace login page, stopping script execution');
        return;
    }
    
    console.log('[LOGIN] Crystal Palace login page detected, starting login automation...');
    ...
    async function performLogin() {
        ...
        // Find login form elements - Crystal Palace specific selectors
        const emailInput = document.querySelector('input[type="email"], input[name="email"], #email, #Email, input[name="username"], #username, input[name="login"], #login, input[name="user"], #user');
        const passwordInput = document.querySelector('input[type="password"], input[name="password"], #password, #Password, input[name="pass"], #pass');
        
        // Try to find the specific Crystal Palace submit button first, then fallback to generic selectors
        let submitButton = document.querySelector('button[class*="standard-button_btn__nWZxu"][type="submit"]');
        if (!submitButton) {
            console.log('[LOGIN] Specific Crystal Palace submit button not found, trying fallback selectors...');
            // Try to find button with "Submit" text and standard-button class
            const allButtons = document.querySelectorAll('button[class*="standard-button"]');
            for (let button of allButtons) {
                if (button.textContent.trim() === 'Submit') {
                    submitButton = button;
                    console.log('[LOGIN] Found submit button by text content');
                    break;
                }
            }
            if (!submitButton) {
                // Try generic selectors
                submitButton = document.querySelector('button[type="submit"], input[type="submit"], .login-button, #login-button, .btn-primary, .btn-login, button[class*="standard-button"], button[class*="submit"], .btn[class*="login"]');
            }
        } else {
            console.log('[LOGIN] Found specific Crystal Palace submit button');
        }
        ...
        console.log('[LOGIN] Submit button text:', submitButton.textContent);
    ...
    async function waitForLoginResult() {
        ...
        return new Promise((resolve, reject) => {
            const checkTimer = setInterval(() => {
                ...
                // Check if we've been redirected to a success page (Crystal Palace specific)
                if (window.location.href.includes('cpfc.co.uk') && 
                    !window.location.href.includes('tm-authorize')) {
                ...
                const errorElements = document.querySelectorAll('.error, .alert-error, .login-error, [class*="error"], .alert-danger, .text-danger');
    ...
    function extractAuthData() {
        ...
        for (const pattern of tokenPatterns) {
            ...
            if (match) {
                const token = match[1];
                localStorage.setItem('crystal_palace_auth_token', token);
        ...
        if (cookies['ASP.NET_SessionId'] || cookies['__RequestVerificationToken'] || cookies['sessionid']) {
            ...
            localStorage.setItem('crystal_palace_session_cookies', JSON.stringify(cookies));
        }
    ...
    function monitorLoginSuccess() {
        // Check if we're on a logged-in page (Crystal Palace specific)
        if (window.location.href.includes('cpfc.co.uk') && 
            !window.location.href.includes('tm-authorize')) {
    ...
    window.getCrystalPalaceAuthToken = function() {
        return localStorage.getItem('crystal_palace_auth_token');
    };
    
    // Helper function to get stored session cookies
    window.getCrystalPalaceSessionCookies = function() {
        const cookies = localStorage.getItem('crystal_palace_session_cookies');
        return cookies ? JSON.parse(cookies) : null;
    };
    
    console.log('[LOGIN] Crystal Palace login content script initialization complete');
})();
